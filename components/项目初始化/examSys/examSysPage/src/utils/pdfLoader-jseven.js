 ...
 * @LastEditTime: 2023-08-02 10:26:19
 * @LastEditors: jseven
 */
...
export class PdfLoader {
  ...
  getPdf() {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      for (let i = 0; i < pages.length; ++i) {
        ...
        this.addImage(baseX, baseY + pdfHeaderH - pages[i], pdf, data, width, height);

        // 将 内容 与 页眉之间留空留白的部分进行遮白处理
        this.addBlank(0, pdfHeaderH, this.A4_WIDTH, baseY, pdf);

        // 将 内容 与 页脚之间留空留白的部分进行遮白处理
        this.addBlank(0, this.A4_HEIGHT - baseY - pdfFooterH, this.A4_WIDTH, baseY, pdf);
        ...
        if (i < pages.length - 1) {
          ...
          // 对多余的内容部分进行遮白
          this.addBlank(0, baseY + imageHeight + pdfHeaderH, this.A4_WIDTH, this.A4_HEIGHT - imageHeight, pdf);
        }
  ...
  getPdfByType(pdf) {
    ...
    switch (this.outputType) {
      ...
      case 'save':
        ...
        break;
      case 'zip':
        // 2024-03-14 如果需要zip，将pdf数据直接返回，然后在外层调用JSZip实现压缩
        result = pdf;
        break;
      default:
  ...
  getItemPosition(node, top) {
    if (node.dataset.position) {
      ...
      console.log('%c [ pageH ]-424', 'font-size:13px; background:pink; color:#bf2c9f;', pageH);

      // y 计算距离当前页顶部高度（当前元素高度 - 上一个页点高度 - 页眉 - 留白）
      let offsetY = top - pageH + pdfHeaderH;
      console.log('%c [ offsetY ]-428', 'font-size:13px; background:pink; color:#bf2c9f;', offsetY);
